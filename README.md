...
- **Budget panel ≈ frontier solo, but the local economics flip:** 27B×2 ≈ solo-35B on quality but
  ~13× slower (one GPU runs the weak model 3× sequentially with no per-token saving).
- **Judge prompt & persona ≈ flat.** A leaked frontier "persona" prompt and elaborate judge prompts
  change *style* (citations, hedging) but not the score; on convergent inputs they were byte-identical.
- **Fusion amplifies the panelists.** With facts in context it reconciles and catches confident
...
- `qwopus-35b-fusion` → full fusion (panelists + judge). Quality.
